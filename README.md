...
metadata for a project that depends on EasyConfig.
...
needs no extra package. On Python 3.10, EasyConfig installs and uses the
backport `tomli`. EasyConfig reads TOML but `save()` always writes JSON, so no
